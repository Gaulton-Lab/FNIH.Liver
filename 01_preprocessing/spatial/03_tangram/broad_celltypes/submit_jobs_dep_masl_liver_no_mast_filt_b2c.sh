...
#JOBID_3=$(sbatch --export=OUT_DIR=${OUT_DIR},CELLTYPE_MD_COL=${CELLTYPE_MD_COL},JOB_OUT=${JOB_OUT},PATIENT_ID=${PATIENT_ID} --output="${JOB_OUT}pp_arr_${PATIENT_ID}.e-%a" --output="${JOB_OUT}pp_arr_${PATIENT_ID}.o-%a" --array=43 Tangram_postPrediction_arr.sh | awk '{print $4}')
