...
# them separately again. Note: for QY_2673 specifically, stage 3 was
# previously run with a hand-picked `--array=21-33` instead of the full
# `1-"$NUM_FILES"` range; edit the --array flag below if reprocessing just
# that patient.
#
# Cleanup note: removed a second, hardcoded-array-range dead alternate for
# stage 2 and a stage-3 dead alternate that had a real bug (--output passed
# twice instead of --error).
#
...
## marker/rank-genes directory, both from the Normal preprocessing run.
...
# ~13-19 min historically. For QY_2673 specifically, this was previously run
# with --array=21-33 instead of the full range (see note above).
