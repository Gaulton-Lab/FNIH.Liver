...
SPA_FP=$(echo "$SAMPLE_LINE" | cut -f3 )
...
#JOBID_2=$(sbatch --export=OUT_DIR=${OUT_DIR},SC_FP=${SC_FP},MARKERS=${MARKERS},CELL_DENSITY=${CELL_DENSITY}  --output="${JOB_OUT}tans_script_gpu_arr${PATIENT_ID}.o-%a" --error="${JOB_OUT}tans_script_gpu_arr${PATIENT_ID}.e-%a" --array=1-4 tans_tangram_train_sp_gpu_arr.sh | awk '{print $4}')
...
#JOBID_3=$(sbatch --export=OUT_DIR=${OUT_DIR},CELLTYPE_MD_COL=${CELLTYPE_MD_COL},JOB_OUT=${JOB_OUT},PATIENT_ID=${PATIENT_ID} --output="${JOB_OUT}pp_arr_${PATIENT_ID}.e-%a" --output="${JOB_OUT}pp_arr_${PATIENT_ID}.o-%a" --array=31-32 Tangram_postPrediction_arr.sh | awk '{print $4}')
