...
#SBATCH --qos=hcg-csd772
#SBATCH --job-name=nl_filt
#SBATCH -e /tscc/nfs/home/cmiciano/job_outs/Liver/RNA/spatial/sub_jobs_liver_nash_filt_ds.sh.e
#SBATCH -o /tscc/nfs/home/cmiciano/job_outs/Liver/RNA/spatial/sub_jobs_liver_nash_filt_ds.sh.o
...
# Driver for the broad-cell-type Tangram pipeline, NASH condition.
#
# This submits the 3 remaining pipeline stages for one sample (one array
# task = one line of the sample sheet), after Split_h5ad_b2c_filt_arr.sh has
# already produced OUT_DIR/subsets/:
...
#
# PIPELINE-STAGE TOGGLE: see Submit_jobs_dep_masl_liver_no_mast_filt_b2c.sh
# for the general pattern (only one stage normally active at a time during
# interactive use). All 3 stages are active below with their dependency
# chain intact -- this file is the complete, "run it all" reference version.
#
# Cleanup note: removed a stage-3 dead alternate that had a real bug
# (--output passed twice instead of --error) and a stray "manually 49-52 for
# broad" one-off array-range override for stage 2.
#
...
## marker/rank-genes directory, both from the NASH preprocessing run.
SC_FP='/tscc/projects/ps-epigen/users/cmiciano/Liver/RNA/outputs/sandbox/spatial/20241108_tangram_preprocessing_genelist50_liver_nash_rem_mast/Preprocessed_adsc.h5ad'
MARKERS='/tscc/projects/ps-epigen/users/cmiciano/Liver/RNA/outputs/sandbox/spatial/20241108_tangram_preprocessing_genelist50_liver_nash_rem_mast/markers'
